...
public class MGameUtility2 {
	...
	public static int[] prioritize(double[] evaluation,int maxHeuristics) {
		...
		Piece[][][] posStack = new Piece[GameSystem.NO_LINES][][];
	...
	private static Piece[][] fillPosition(Piece[][] pos0, boolean white, double[] eval, int[] evalList, Objectives o, BlockingQueue<Message> queue) {
	    ...
	    double temp;
		int closenessTld = 7;		
	    int d = GameSystem.DIST_THRESHOLD;
	    ...
	    while(genTries > 0){
	        ...
	        for(int i = 0; i<maxTries; i++) {
		        if(dir == -1) {
			        if(GameSystem.MeasureHeuristic(pos, white, heur) > GameSystem.MeasureHeuristic(pos0, white, heur)) {
			        	break;
			        }
		        }
		        else if(dir == 1) {
		...
		for(int i = 1; i < evalList.length; i++) {
		   //6x passes of b9:
		    ...
		    while(passTries > 0){
		    	...
		    	pos2 = pos;
		        while ((temp == 1 && GameSystem.MeasureHeuristic(pos0, white, evalList[i]) < GameSystem.MeasureHeuristic(pos2, white, evalList[i]) ||
		        		temp == -1 && GameSystem.MeasureHeuristic(pos0, white, evalList[i]) > GameSystem.MeasureHeuristic(pos2, white, evalList[i])) &&
		        		passTries > 0){
